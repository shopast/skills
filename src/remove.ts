...
export async function removeCommand(skillNames: string[], options: RemoveOptions) {
  ...
  if (options.all && namedSkills.length > 0) {
    ...
    p.log.info(
      'Use `skillycli remove --all` to remove every skill, or omit --all to remove only the named skills.'
    );
    p.log.info(`Example: skillycli remove ${namedSkills[0]} -y`);
    process.exit(1);
  }
 ...
 *
 * Supports both positional names (`skillycli remove foo`) and `-s/--skill`
